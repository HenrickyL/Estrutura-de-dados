...
using namespace std;
//Global
const int tam[] = {1000,5000,10000,25000,50000,65000,85000,100000,150000,200000};//,500000,1000000};
...
int main(){
	...
	//////ifstream arqIn;
 	...
 	arqOut.open("dadosHeap.txt",ios::out); //escrita
	...
	for(int k: tam){ //for percorrendo os tamanhos
		...
		for(int l =0;l<N_media;l++){ //for para fazer as médias
			...
			std::cout<<"-------------\n::Heap_SORT::\n";
		//interativo
			somaI += TimeHeapInterative(v, n);
		//recursivo
			SomaR += TimeHeapRecursive(v, n);
			if(l == N_media-1){
...
//	verificações
	...
	if((double) timeFinal > 60*60){
		...
		arqOut.open("Tempo totalHeap.txt",ios::out);
		arqOut << timeFinal <<" h \n";
		arqOut.close(); 
