...
using namespace std;
//Global
const int tam[] = {1000,5000,10000,25000,50000,65000,85000,100000};//,500000,1000000};
...
int main(){
	...
	//////ifstream arqIn;
 	...
 	arqOut.open("dadosInsertionList.txt",ios::out); //escrita
	...
	for(int k: tam){ //for percorrendo os tamanhos
		...
		for(int l =0;l<N_media;l++){ //for para fazer as médias
			...
			for(int i=0;i<n;i++){ // for para gerar os valores e add a lista
				//v[i] = rand();
				list->push_back(rand());
			}
		...
		//salvo
		aux = to_string(n)+"\t"+to_string(mediaI);
		arqOut << aux <<"\n";
...
//	verificações
	...
	if((double) timeFinal >= 60*60){
		...
		arqOut.open("Tempo totalInsertionList.txt",ios::out);
		arqOut << timeFinal <<" h \n";
		arqOut.close(); 
